...
- **`EPHEMERIS_SERVER_PLAN.md`** and **`EPHEMERIS_CLIENT_PLAN.md`** — the
  ...
  limits and tokens, packaging -- with go-live waiting on a
  host; the operator's guide is `ephsrv/deploy/README.md`.
  **The wire protocol is version 4** since 2026-09-17, and its
  specification is `EPHEMERIS_PLUGINS_PLAN.md` §3, not the server plan's §4
  any more: source-neutral (NAIF/SPK-ID bodies, per-object profiles,
  two-part Julian instants), so an engine other than the Swiss Ephemeris can
  serve it. `ephsrv/ephproto.h` is the codec both ends compile and Ephemeris
  Prometheia vendors; `ephsrv/conformance/` holds the fixtures that pin it,
  generated by `tools/ephproto4-fixtures.py` from the specification alone, and
  `ephsrv/registries.json` is Appendix A as data, generated by
  `tools/gen-registries.py` and diffed by `make check`.
  **Version 4 is not locked yet**, and the work is on branch `ephv4`, not
  here: one drop remains (a `corrApplied` byte in META, a §3.4 contradiction
