...
./run-qt-tests.sh                # 3176 assertions + startup checks
...
commit. Current state: **3176 passed, 0 failed**, startup diagnostics ok. The full suite is also clean under AddressSanitizer (`make -f Makefile.qt.asan`).
...
and locations, all 338 menu items fire without crashing, 258/258 Windows menu
