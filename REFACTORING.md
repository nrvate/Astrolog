...
| O2 | the rest of the object core: planet[]/chouse[] aliases (539 refs), force/rgobjset/kObjA | the same mechanism, mechanically: a GRD* variant per element type, `.rgn` at the pointer boundaries. planet/chouse are `cp0.obj`/`cp0.cusp`, struct members of CP, so their guard has to live in the struct and 16 `cp*.obj`-as-pointer sites take `.rgn` | **pending, mechanical** |
