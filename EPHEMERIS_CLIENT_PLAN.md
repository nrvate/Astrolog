...
- **Increment 4 is built** (work log item 6): the required-server dialog
  and exit ladder (`EXIT_NO_EPHEMERIS` 86), the About status line, and
  §7's address field. It is the only piece with user-visible blocking UI
  -- the maintainer tests such changes by hand, so it sits uncommitted
  on branch `ephclient4` until he confirms.
...
6. **Increment 4 built: the required-server dialog, the exit ladder, the
   status line, and the address field** (branch `ephclient4`, 2026-09-16,
   uncommitted -- blocking UI, so the maintainer tests it by hand before
   it lands). With the backend selected and no local ephemeris anywhere
