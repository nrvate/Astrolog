...
#
#   versions  a version-3 client is welcomed in 3, a version-2 client in 2
#             and served; a version-1 client gets ERROR 8 it can read, in
#             its own envelope version, telling it to update
...
#
# The golden gate's PROTO=2 run is the compatibility half: a version-2
# client bit-exact against this server.
#
...
ask() {   # ask EXPECTED-EXIT LABEL PORT CLIENT-ARGS...
  ...
  set +e
  "$CLI" --port "$port" --objs 0,1 --count 2 "$@" > "$S/cli.out" 2> "$S/cli.err"
  local got=$?
  set -e
...
ask 0 "a version-3 client" "$PORT"
grep -q "^WELCOME v3 " "$S/cli.out" || fail "a version-3 client was not welcomed in 3: $(cat "$S/cli.out")"
ask 0 "a version-2 client" "$PORT" --proto 2
grep -q "^WELCOME v2 " "$S/cli.out" || fail "a version-2 client was not welcomed in 2: $(cat "$S/cli.out")"
ask 2 "a version-1 client" "$PORT" --proto 1
grep -q "server ERROR 8 (HELLO, v1):.*update" "$S/cli.err" ||
  fail "a version-1 client did not get a readable ERROR 8: $(cat "$S/cli.err")"
echo "  versions 3 and 2 welcomed in their own version; 1 told to update, in 1"

# -- hello -----------------------------------------------------------------
ask 2 "a REQUEST before HELLO" "$PORT" --no-hello --quiet
grep -q "server ERROR 1 .*REQUEST before HELLO" "$S/cli.err" ||
...
"$CLI" --port "$P1" --objs 0 --count 1 --quiet --idle-ms 2000 > /dev/null 2>&1 & H1=$!
"$CLI" --port "$P1" --objs 0 --count 1 --quiet --idle-ms 2000 > /dev/null 2>&1 & H2=$!
PIDS+=("$H1" "$H2")
sleep 0.5
...
"$CLI" --port "$P2" --objs 0 --count 1 --quiet --idle-ms 2000 > /dev/null 2>&1 & H1=$!
PIDS+=("$H1")
sleep 0.5
