// Vendored from cgif (https://github.com/dloebl/cgif), tag v0.5.4, commit
// 2dfb49c22fbe3894e46731f7e3b6ffa84311e44e -- the newest tag on 2026-09-14.
// MIT licensed; the licence is cgif-license.txt. Only the indexed-image API
// is vendored (cgif.h, cgif.c, cgif_raw.h, cgif_raw.c); the RGB quantizing
// cgif_rgb.c is not. Astrolog writes animated GIFs through it: see
// FWriteGifFrame() in xdevice.cpp.
//
// Renamed from .c to .cpp so every build here compiles it with the rule it
// already has, as the Swiss Ephemeris files are. C++ needed these edits
// and nothing else, all mechanical:
//   - a cast on each malloc() result (void * does not convert implicitly);
//   - a cast from int to cgif_result where cgif_raw_close()'s or
//     flushFrame()'s int result is stored.
